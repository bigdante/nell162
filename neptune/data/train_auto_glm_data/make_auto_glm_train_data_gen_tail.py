...
ms = ["get_entities()", "choose_a_head()", "determine_types()", "choose_a_type()", "get_relations()", "choose_a_relation()", "get_a_alias()",
      "choose_a_tail()", "verify()"]
gs = ['get_sentences']
...
head_template = "[A] ^{s[head_id]}【choose_a_head()】\n" \
                ...
                "\n" \
                "[T] ^{s[head_id]} Chosen a HEAD, determine TYPES of HEAD according to SENTENCE.\n" \
                "[A] ^{s[head_id]}【determine_types()】\n" \
...
type_template = "[A] ${s[type_index]}【choose_a_type()】\n" \
                ...
                "\n" \
                "[T] ${s[type_index]} Discover relations HEAD has based on TYPE.\n" \
...
relation_template = "[A] &{s[relation_index]}【choose_a_relation()】\n" \
                    ...
                    "\n" \
                    "[T] &{s[relation_index]} Received a RELATION, get ALIAS of RELATION.\n" \
                    "[A] &{s[relation_index]}【get_alias()】\n" \
                    "[R] &{s[relation_index]} ALIAS={s[relation_alias]}\n" \
                    "\n" \
                    "[T] &{s[relation_index]} Extracted ALIAS, run through ALIAS.\n" \
                    "{s[alias_template]}"
                    # "[T] &{s[relation_index]} ALL ALIAS run through done. Now vote.\n" \
                    # "[A] &{s[relation_index]}【vote()】\n" \
                    # "[R] &{s[relation_index]} TAILS={s[tails]}\n" \
                    # "\n" \
                    # "[T] &{s[relation_index]} After voting, time to check all TAILS.\n"

alias_template = "[A] @{s[alias_index]}【get_a_alia()】\n" \
                 "[R] @{s[alias_index]} ALIA={s[alia]}\n" \
                 "\n" \
                 "[T] @{s[alias_index]} Using ALIA, answer the question: {s[alia_template]}\n" \
                 "[A] @{s[alias_index]}【get_tails()】\n" \
...
def make_iter_data():
    with open(f"./train_auto_glm_data/{type}/ori_gen_tails.json", "w") as f:
        for sample in tqdm(data[:100]):
            ...
            for l in labels:
                ...
                heads = ""
                for head_id, head in enumerate(in_sentence_entity):
                    ...
                    for t_id, h_type in enumerate(head_types):
                        ...
                        for r_id, relation in enumerate(relations):
                            ...
                            relation_s['relation_alias'] = str(list(r_t.keys()))
                            alias_templates = ""
                            vote = []
                            for a_id, alia in enumerate(r_t):
                                ...
                                s = {
                                    "sentence": sentence,
                                    "head": head_name,
                                    "tail_choose": in_sentence_entity
                                }
                                alias_s["alia_template"] = r_t[alia].format(s=s)
...
def make_rounds_thought_chat(input_file_path):
    with open(f"./train_auto_glm_data/{type}/rounds_chat_gen_tails.json", "w") as outfile:
        with open(input_file_path, 'r') as f:
            ...
            for data in tqdm(datas, desc="making round"):
                ...
                for i, block in enumerate(blocks):
                    if i > len(blocks) - 3:
                        break
                    if any(word in block for word in gs):
                        continue
                    block_dict = {
                    ...
                    if "[A]【get_sentences()】" not in blocks[i + 1]:
...
if __name__ == '__main__':
    type = "step"
    make_iter_data()
    make_rounds_thought_chat(f"./train_auto_glm_data/{type}/ori_gen_tails.json")
