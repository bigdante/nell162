...
gs = ['get_sentences', 'search_engine']
...
head_template = "[A] ^{s[head_id]}【choose_a_head()】\n" \
                ...
                "\n" \
                "[T] ^{s[head_id]} Chosen a HEAD, determine TYPES of HEAD may have according to SENTENCE.\n" \
...
engine_template = "[A] #{s[tail_index]}【choose_a_tail()】\n" \
                  ...
                  "\n" \
                  "[T] #{s[tail_index]} utilize the SEARCH_INFO, finally verify the TAIL. Is true or false that \"{s[head]}\" {s[relation]} \"{s[tail]}\"?\n" \
                        ...
                        for r_id, relation in enumerate(relations):
                            ...
                            for t_id, tail in enumerate(result):
                                engine_s = {"tail_index": len(result) - t_id - 1, "context": call_es((head_s['head'], tail)).strip(), 'tail': tail,
                                            "label": "True" if tail == tail_name else "False", "head": head_name, "relation": relation}
        ...
        with open(input_file_path, 'r') as f:
            ...
            for data in tqdm(datas):
                ...
                for i, block in enumerate(blocks):
                    ...
                    if "[A]【search_engine()】" not in blocks[i + 1]:
