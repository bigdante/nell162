        ...
        while True:
            ...
            if trainer.is_world_process_zero():
                if training_args.predict_with_generate:
                    ...
                    prediction_m = predictions[0]
                    # label_m = labels[0]
                    match = re.search(r'【([^【,]+)', prediction_m)
                    if match:
                        ...
                        for f in functions:
                            if f in result:
                                ...
                                flag = 1
                        if not flag:
                            print("not method match")
                            continue
                    else:
                        print("not method offered")
                        continue
                    next_input = tokenizer.decode(predict_dataset[0]["input_ids"]) + "\n[Return] " + method_return
