...
def make_data():
    with open(f"make_COT_traindata_redocred/{type}/ori.json", "w") as f:
...
def make_rounds_thought_chat(input_file_path):
    with open(f"make_COT_traindata_redocred/{type}/rounds_chat.json", "w") as outfile:
        with open(input_file_path, 'r') as f:
...
if __name__ == '__main__':
    ...
    make_iter_data()
    # if type == "one_thought":
    #     make_rounds_thought_chat(f"./make_COT_traindata_redocred/{type}/ori.json")
    # else:
    #     make_rounds_no_thought_chat(f"./make_COT_traindata_redocred/{type}/ori.json")
    #
    # split_json_file(f"./make_COT_traindata_redocred/{type}/rounds_chat.json", f"../ChatGLM-6B-main/ptuning/auto_kg/{type}/train.json",
