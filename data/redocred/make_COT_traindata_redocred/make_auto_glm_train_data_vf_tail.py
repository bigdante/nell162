...
def make_iter_data():
    with open(f"make_COT_traindata_redocred/{type}/ori_vf_tails.json", "w") as f:
...
def make_rounds_thought_chat(input_file_path):
    with open(f"make_COT_traindata_redocred/{type}/rounds_chat_vf_tails.json", "w") as outfile:
...
if __name__ == '__main__':
    type = "step"
    make_iter_data()
    make_rounds_thought_chat(f"make_COT_traindata_redocred/{type}/ori_vf_tails.json")
    split_json_file(f"make_COT_traindata_redocred/{type}/", f"../ChatGLM-6B-main/ptuning/auto_kg/{type}/train.json",
